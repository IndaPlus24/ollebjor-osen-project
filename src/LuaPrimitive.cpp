...
LuaPrimitive::LuaPrimitive(PrimitiveType type, LuaVector3& position) {
    this->m_ref = SceneManager::Get().AddEntity(
...
int LuaPrimitive::luaGetType(lua_State* L) {
    LuaPrimitive* self =
        MetatableRegistry::Get().check_userdata<LuaPrimitive>(L, 1);
...
int LuaPrimitive::luaNew(lua_State* L) {
    ...
    if (lua_isnumber(L, 1)) {
        ...
        PrimitiveType type = IntToPrimitiveType(typeInt);
        MetatableRegistry::Get().create_and_push<LuaPrimitive>(L, type);
        return 1;
    }
    MetatableRegistry::Get().create_and_push<LuaPrimitive>(
