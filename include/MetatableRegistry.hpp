...
class MetatableRegistry {
  ...
  public:
    ...
    template <typename T>
    void register_metatable(const std::string& name,
    ...
    const char* get_metatable_name(const std::type_index& type) const {
        ...
        if (it != metatable_registry.end()) {
            ...
            throw std::runtime_error("Metatable not registered for type " +
                                     std::string(type.name()));
        }
    }

    // Get the metatable name for a C++ type
    template <typename T> const char* get_metatable_name() const {
    ...
    template <typename T> T* check_userdata(lua_State* L, int index) const {
        ...
        bool is_reference =
            meta_it != type_metadata.end() && meta_it->second.is_reference_type;
